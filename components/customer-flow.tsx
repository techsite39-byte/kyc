...
function ImageInput({ capture, multiple, label, onChange }: { capture?: "environment" | "user"; multiple?: boolean; label?: string; onChange: (event: ChangeEvent<HTMLInputElement>) => void }) {
  return <label className="grid min-h-48 cursor-pointer place-items-center rounded-2xl border-2 border-dashed border-slate-300 p-6 text-center text-sm font-semibold text-brand-600"><span>{label || (capture === "user" ? "Upload Photo" : "Upload Image")}</span><input className="hidden" type="file" accept="image/*" capture={capture} multiple={multiple} onChange={onChange} /></label>;
}
